...
{
    [Table("WMS_IN_RECEIPT_IQC_RECORD_HIS")]
    [Index(nameof(iqcRecordNo), IsUnique = true)]
    public class WmsInReceiptIqcRecordHis : BasePoco
	{
        /// <summary>
        /// 主键ID
        /// </summary>
		...
		[Column("BAD_OPTIONS")]
        [Display(Name = "不良选项")]
        [StringLength(255, ErrorMessage = "不良选项长度不能超出255字符")]
		public string badOptions { get; set; }

		/// <summary>
		/// 不良处理方式
		/// </summary>
		[Column("BAD_SOLVE_TYPE")]
        [Display(Name = "不良处理方式")]
        [StringLength(255, ErrorMessage = "不良处理方式长度不能超出255字符")]
		...
		//[StringLength(500, ErrorMessage = "详细说明长度不能超出500字符")]
        ...
        [StringLength(100, ErrorMessage = "检验记录单号长度不能超出100字符")]
		public string iqcRecordNo { get; set; }

        /// <summary>
        /// 检验结果状态;检验结果状态：0初始创建，21质检中（部分合格部分不合格），90质检完成（ERP同步质检结果后）；92删除；93强制完成
        /// </summary>
        [Display(Name = "检验结果状态")]
        //[Column("IQC_STATUS")]
        [Column("IQC_RECORD_STATUS")]
		[Required]
		//[StringLength(50, ErrorMessage = "检验结果状态;检验结果状态：0初始创建，21质检中（部分合格部分不合格），90质检完成（ERP同步质检结果后）；92删除；93强制完成长度不能超出50字符")]
		//public string iqcStatus { get; set; } = "0";
		public int iqcRecordStatus { get; set; } = 0;
		...
		/// 包装数量
		/// </summary>
		//[Column("PKG_QTY", TypeName = "decimal(18,3)")]
		...
		[Column("ERP_QUALIFIED_SPECIAL_QTY", TypeName = "decimal(18,3)")]
        [Display(Name = "ERP特采数量")]
        public decimal? erpQualifiedSpecialQty { get; set; } = 0;
		...
		[Column("WMS_UNQUALIFIED_QTY", TypeName = "decimal(18,3)")]
        [Display(Name = "检验不合格数量")]
        public decimal? wmsUnqualifiedQty { get; set; } = 0;

		/// <summary>
		/// ERP不合格数量
		/// </summary>
		//[Column("UNQUALIFIED_SPECIAL_QTY", TypeName = "decimal(18,3)")]
		[Column("ERP_UNQUALIFIED_QTY", TypeName = "decimal(18,3)")]
        [Display(Name = "ERP不合格数量")]
        //public decimal? unqualifiedSpecialQty { get; set; } = 0;
        public decimal? erpUnqualifiedQty { get; set; } = 0;
